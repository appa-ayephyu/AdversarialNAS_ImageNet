...
def main():
    ...
    # logger.info('init')
    # logger.info('alpha_normal = %s', str(F.softmax(gen_net.module.alphas_normal, dim=-1)))
    # logger.info('alpha_up = %s', str(F.softmax(gen_net.module.alphas_up, dim=-1)))
    # logger.info('dis_alpha_normal = %s', str(F.softmax(dis_net.module.alphas_normal, dim=-1)))
    # logger.info('dis_alpha_down = %s', str(F.softmax(dis_net.module.alphas_down, dim=-1)))
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch_D)), desc='total progress'):
        ...
        if epoch == 0 or epoch % args.val_freq == 0 or epoch == int(args.max_epoch_D) - 1:
            ...
            #     print_alpha_normal = F.gumbel_softmax(F.softmax(gen_net.module.alphas_normal, dim=-1), tau=tau, hard=False, dim=-1)
            #     print_alpha_up = F.gumbel_softmax(F.softmax(gen_net.module.alphas_up, dim=-1), tau=tau, hard=False, dim=-1)
            #     print_dis_alpha_normal = F.gumbel_softmax(F.softmax(dis_net.module.alphas_normal, dim=-1), tau=tau, hard=False, dim=-1)
            #     print_dis_alpha_down = F.gumbel_softmax(F.softmax(dis_net.module.alphas_down, dim=-1), tau=tau, hard=False, dim=-1)
            # else:
            #     print_alpha_normal = F.softmax(gen_net.module.alphas_normal, dim=-1)
            #     print_alpha_up = F.softmax(gen_net.module.alphas_up, dim=-1)
            #     print_dis_alpha_normal = F.softmax(dis_net.module.alphas_normal, dim=-1)
            #     print_dis_alpha_down = F.softmax(dis_net.module.alphas_down, dim=-1)

            # logger.info('alpha_normal = %s', str(print_alpha_normal))
            # logger.info('alpha_up = %s', str(print_alpha_up))
            # logger.info('dis_alpha_normal = %s', str(print_dis_alpha_normal))
            # logger.info('dis_alpha_down = %s', str(print_dis_alpha_down))
